...
static void port_serial_rx_interrupt(hal_ser_dev_t dev)
{
    ...
    if((errorflags == RESET) && ((isrflags & USART_ISR_RXNE_RXFNE) != RESET))
    {
    ...
    while(errorflags != RESET)
    {
        ...
        errorflags = (isrflags & (USART_ISR_PE | USART_ISR_FE | USART_ISR_ORE | USART_ISR_NE));
    }
}
...
//static void port_serial_configure(hal_ser_dev_t dev, uint32_t baudrate, hal_ser_data_size_t data_size,
//                                 hal_ser_parity_t parity, hal_ser_stop_bit_t stop_bits,
...
//    cfg.PrescalerValue = LL_USART_PRESCALER_DIV1;
//
...
static void port_serial_configure(hal_ser_dev_t dev, uint32_t baudrate, hal_ser_data_size_t data_size,
                                 ...
                                 hal_ser_flow_control_t flow_control)
{
    // REMOVA LL_USART_Disable e LL_USART_Init
    // A UART já está configurada pelo CubeMX

    // Apenas atualize configurações específicas se necessário
    // Por exemplo, se precisar mudar baud rate dinamicamente:
    LL_USART_SetBaudRate(UART8, HAL_RCC_GetPCLK1Freq(), LL_USART_PRESCALER_DIV1,
...
{
	if(interrupt_cbk[dev])
		return 0; // Or a different error code, as uint16_t can't be -1
...
//    cfg.PrescalerValue = LL_USART_PRESCALER_DIV1;
//
//    LL_USART_Init(UART8, &cfg);
//    LL_USART_Enable(UART8);
//
//    // 5. Configurar interrupção
...
static void port_serial_init(void)
{
    // REMOVA toda a configuração de hardware (pinos, clock, reset)
    // Mantenha apenas a configuração de interrupção e buffer
    
    // 1. Configurar interrupção (já feita pelo CubeMX, mas garantimos o estado)
    port_serial_interrupt_state_set(HAL_SER_DEV_DEBUG, false);
    port_serial_interrupt_init(HAL_SER_DEV_DEBUG);
    port_serial_flush(HAL_SER_DEV_DEBUG);

    // 2. Não configure pins, clocks ou UART - já foi feito pelo CubeMX
}
